...
WITH PTCol
  ...
  --ORDER BY TableName, SPAR.partition_number, SIDX.index_id, FileGroupName  
   ...
   SELECT *
      FROM DBS
      WHERE
       -- (TableName = 'td_PMHT')  AND 
       -- (FileGroup LIKE 'Staging_%_2015') AND
       ( rows > 0 AND total_pages > 0 )
       AND -- (IType = 0) AND 
       --((Compression = 'NONE') OR (Scheme IS NULL)) AND
       --((Compression = 'NONE') OR ((Scheme IS NULL) OR (Scheme NOT LIKE '%CYCLE%'))) AND
       -- (TableSchema = 'cdc') AND
       --(TableName Like 'PDR%') AND
       ( total_pages > 0 ) --AND 
      ...
      ORDER BY
       ...
       [P#],
       [FileGroup];
--ORDER BY unused_pages DESC,  [TotalSizeMB] DESC, TableSchema, TableName, [IDX#], [P#], [FileGroup];
--ORDER BY rows desc, TableSchema, TableName, [IDX#], [P#], [FileGroup];

...
;WITH FullSID
	 AS (SELECT SP.sid, 
				...
				CONCAT( 'S-',
						CONVERT(VARCHAR(3), CONVERT(INT, SUBSTRING(SP.sid, 1, 1))), '-',	-- SID version
						CONVERT(VARCHAR(15), CONVERT(INT, SUBSTRING(SP.sid, 3, 6))),		-- SID authority group
						(SELECT	TOP (10) '-'+ CONVERT(VARCHAR(12), CONVERT(BIGINT,CONVERT(BINARY(4), REVERSE(CONVERT(BINARY(4), SUBSTRING(SP.sid, (POS.GrpNum * 4) + 5, 4)))))) -- Sid auth sub group
							FROM ( SELECT ROW_NUMBER() OVER(ORDER BY ( SELECT NULL )) AS GrpNum FROM sys.objects ) POS  -- Return a number sequence 1, 2, 3, 4, ...
							WHERE (POS.GrpNum <= CONVERT(INT, SUBSTRING(SP.sid, 2, 1))) -- Number of Sid authority subgroups
							ORDER BY GrpNum
							FOR XML PATH('')
							)						
						) AS BigSid,
			 ...
			 WHERE(SP.[type] IN('U', 'G'))  -- Only consider Windows User or Widnows Group
		)  

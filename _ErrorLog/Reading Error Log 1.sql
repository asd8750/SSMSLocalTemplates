...
UPDATE @ErrorLogs
   SET ReadThis = CASE
                      ...
                      WHEN n.nextlogdate >= DATEADD(
                                                   DAY,
                                                   DATEDIFF(DAY, '20100101', GETDATE()) - ABS(@daysback),
                                                   '20100101'
                                                   ) THEN
                          1
                      ELSE
        ...
        (( ProcessInfo = 'Server' ) -- we like server-related boot messages.
         ...
         OR
             ( -- filter out noisy messages that we don't really need to see here
                 ...
                 AND Text NOT LIKE 'DbMgrPartnerCommitPolicy::SetSyncAndRecoveryPoint: %'
                 AND Text NOT LIKE 'DbMgrPartnerCommitPolicy::SetSyncState: %'
                 AND Text NOT LIKE 'The Service Broker endpoint is in disabled or stopped state%'
